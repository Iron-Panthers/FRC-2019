...
public class JoystickWrapper extends Joystick {
	...
	/**
	 * calculates right and left powers based on the input y value and a
	 * radius calculated based on the input x value
	 * @param tx
	 * @param ty
	 * @return double[] powers
	 */
	double[] radialDrive(double tx, double ty) {
		double turnPower = tx;
		...
		double turnRadius = - Math.log(Math.abs(turnPower));
	...
	public void findMagnitude() {
		...
		if(Math.abs(y) > Math.abs(x) * Constants.Input.VERTICAL_BOWTIE_DEADZONE_SLOPE) {
			...
			x = (x - (Math.abs(y) / Constants.Input.VERTICAL_BOWTIE_DEADZONE_SLOPE)) / (1 - (Math.abs(y) / Constants.Input.VERTICAL_BOWTIE_DEADZONE_SLOPE));
		}
		if(Math.abs(x) > Math.abs(y) * Constants.Input.HORIZONTAL_BOWTIE_DEADZONE_SLOPE) {
			y = 0;
		}
		else {
			y = (y - (Math.abs(x) / Constants.Input.HORIZONTAL_BOWTIE_DEADZONE_SLOPE)) / (1 - (Math.abs(x) / Constants.Input.HORIZONTAL_BOWTIE_DEADZONE_SLOPE));
		}
